...
dtm_df$date1 <- ifelse(nchar(dtm_df$date1) == 5, paste("0", dtm_df$date1, sep = ""), dtm_df$date1) # adds one leading zero if date has only 5 characters
dtm_df$date1 <- ifelse(nchar(dtm_df$date1) == 4, paste("00", dtm_df$date1, sep = ""), dtm_df$date1) # adds two leading zeros if date has only 4 characters
dtm_df$date1 <- ifelse(nchar(dtm_df$date1) == 3, paste("000", dtm_df$date1, sep = ""), dtm_df$date1) # adds three leading zeros if date has only 3 characters

dtm_df$date1 <- paste("20", dtm_df$date1, sep = "") # adds "20" in front of date to get format YYYYMMDD

## creating two additional data formates
dtm_df$date2 <- paste(substr(dtm_df$date1, 7, 8), # reformats to DD.MM.YYYY
     substr(dtm_df$date1, 5, 6),
     substr(dtm_df$date1, 1, 4),
     sep = "."
)
dtm_df$date3 <- as.numeric(paste(substr(dtm_df$date1, 1, 4), # reformats to YYYYMM for merging with monthly data
     substr(dtm_df$date1, 5, 6),
     sep = ""
))
# edit(dtm_df)
...
### change the
mro_ecb <- read.csv2("/Users/gerhardlaimer/Desktop/mcf-seminar/MRO_ECB Data Portal_2025.csv",
     header = TRUE, sep = ";",
...
relevant_rows <- as.numeric(rownames(mro_ecb))[mro_ecb$DATE %in% dtm_df$date2] + 7 # numeric for matching dates + 7 days
relevant_mro <- mro_ecb[relevant_rows, ] # extracts rows 7 days later

mro_dat <- data.frame(
     cbind(
          mro_ecb$DATE[as.numeric(rownames(mro_ecb))[mro_ecb$DATE %in% dtm_df$date2]],
...
### Main LASSO command!!!
############################
fitlin <- gamlr(dtm_df[est_window, rhs_vars], # Independent variables
     dtm_df$MRO_rate[est_window], # Dependent variable
     lmr = 0.01
) # lmr=0.01 is the step-size for which wee look for differenet lambda parameters
...
plot(
     x = predict(fitlin, dtm_df[, rhs_vars]), y = dtm_df$MRO_rate,
...
plot(predict(fitlin, dtm_df[, rhs_vars]),
     type = "l", col = "red",
...
axis(
     side = 1, at = c(0, 50, 100, 150, 200, 250),
